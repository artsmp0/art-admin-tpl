...
type GetUnionArgs<T extends string | undefined> = T extends `${infer _L}{${infer M}}${infer R}` ? M | GetUnionArgs<R> : never
/** 自动计算路径参数 */
...
export interface MakeRequestFn {
    /** 仅传递接口返回的结果类型 */
    <Payload>(config: RequestConfig): (params?: any, restConfig?: RequestConfig) => Promise<MakeRequestRes<Payload>>
    /** 传递接口 querystring 请求参数和返回结果类型 */
    <Payload, Params>(
      config: Partial<Omit<RequestConfig, 'params'>> & { params?: Params }
    ): (params?: Params, restConfig?: RequestConfig) => Promise<MakeRequestRes<Payload>>
    /** 传递接口 querystring 请求参数或 post data 请求参数和返回结果类型 */
    <Payload, Params, Data>(
      config: Partial<Omit<RequestConfig<Data>, 'params'>> & { params?: Params }
    ): (params?: Data, restConfig?: RequestConfig) => Promise<MakeRequestRes<Payload>>
    ...
    ): (params?: GetFinalArgs<ArgsURL>, restConfig?: RequestConfig) => Promise<MakeRequestRes<Payload>>
}
